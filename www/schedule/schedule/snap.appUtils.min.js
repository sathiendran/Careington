...
(function(global, $) {
    snap.namespace("snap.hub")
        .use(["snap.hub.hubModel", "snap.common.overlay"])
        .define("sessionLimiterHub", function($hubModel, $overlay) {
            ...
            var sessionConsultConnection = $.hubConnection();
            var sessionRoomConHub =sessionConsultConnection.createHubProxy('sessionLimiterHub');
            if(!snap.userSession == 'undefined' && snap.userSession != '') {
                 sessionConsultConnection.url = snap.baseUrl + "/api/signalR/";
                 sessionConsultConnection.qs = {
                   "Bearer": snap.userSession.token,
                   // "isMobile": true,
                 };
            }
            sessionRoomConHub.on("onConsultationReview", function() {
             // alert("The Provider is now reviewing the intake form.");
              $scope.$digest();
            });
            sessionRoomConHub.on("onCustomerDefaultWaitingInformation", function() {
              $scope.$digest();
            });
            sessionRoomConHub.on("onConsultationStarted", function() {
              $scope.$digest();
            });
            sessionConsultConnection.logging = true;
            window.whub = sessionConsultConnection;
            sessionConsultConnection.start({
              withCredentials: false
            }).then(function() {
                sessionConsultConnection.disconnected(function() {
                    // console.log("hhhh");
                  setTimeout(function() {
            ...
            sessionRoomConHub.on("onSessionTerminated", function(ip) {
                 navigator.notification.alert(
              ...
              sessionRoomConHub.on("sessionRegistered", function(ip) {

...
(function($, snap) {
    ...
    snap.namespace("snap.service").using(["snapHttp", "snapNotification"])
        .define("groupsService", function($http, $snapNotification) {
            ...
            var apiPath = snap.baseUrl + "/api/v2/clinicians/groups";
            ...
            this.unassignGroup = function(groupId, staffAccounts) {
                ...
                staffAccounts.forEach(function(clinicianId) {
                    var path = [snap.baseUrl + "/api/v2/clinicians/groups", groupId, "clinicians", clinicianId].join("/");
...
(function($, snap) {
    snap.namespace("snap.service").using(["snapHttp"]).define("staffAccountService", function($http) {
        ...
        this.getAccountInfo = function(staffUserId) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/staffprofiles/{0}', staffUserId);
            return $http.get(path);
        };
        this.deleteProfile = function() {
            ...
            return $.ajax({
                type: "DELETE",
                url: snap.string.formatString(snap.baseUrl + '/api/v2/clinicians/staffprofiles/{0}', snap.staffUserId),
                contentType: "application/json; charset=utf-8",
                dataType: "json"
            });
        };
        this.reActivate = function() {
            return $.ajax({
                type: 'PUT',
                url: snap.string.formatString(snap.baseUrl + '/api/v2/clinicians/staffprofiles/{0}/reactivate', snap.staffUserId),
                contentType: "application/json; charset=utf-8",
                dataType: "json"
            });
        }
        this.editAccountInfo = function(staffUserId, data) {
            var path = snap.string.formatString(snap.baseUrl + '/api/v2/clinicians/staffprofiles/{0}', staffUserId);
            return $.ajax({
                type: "PUT",
                url: path,
                contentType: "application/json; charset=utf-8",
        ...
        this.addAccount = function(data) {
            var path = snap.baseUrl + '/api/v2/clinicians/pendingstaffprofile';
            return $.ajax({
                type: "POST",
                url: path,
                contentType: "application/json; charset=utf-8",
        ...
        this.getAllStaffAccountsForScheduling = function(data) {
            var path = [snap.baseUrl + "/api/v2.1/providers/", snap.hospitalSession.hospitalId, "/clinicians?roleFunctions=", snap.security.conduct_virtual_consultations].join("");
        ...
        this.getIndividualTags = function(staffUserId) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/{0}/tags', staffUserId);
            return $http.get(path);
        };
        this.assignTag = function(adiminUserId, staffUserId, hospitalId, tag) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/staffs/{0}/tags', adiminUserId);
            var data = {
                staffId: staffUserId,
        ...
        this.unassignTag = function(tagId, staffUserId) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/tags/{0}/staff', tagId);
        ...
        this.getIndividualRoles = function(staffUserId, hospitalId) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/{0}/hospitals/{1}/roles', staffUserId, hospitalId);

            return $http.get(path);
        };

        this.assignRole = function(roleId, staffUserId) {
                var path = snap.string.formatURIComponents(snap.baseUrl + "/api/v2/clinicians/roles/{0}/staff", roleId);
                var data = [{ userId: staffUserId, isActivated: true }];
        ...
        this.validateClinicianLimit = function(staffUserId, hospitalId, roles) {
            var path = snap.string.formatURIComponents(snap.baseUrl + '/api/v2/clinicians/{0}/limit', staffUserId);
        ...
        this.hasUserInteractions = function(roles) {
            var path = snap.baseUrl + '/api/v2/clinicians/patientinteraction';
            return $.ajax({
                type: "GET",
                url: path,
                data: "roles=" + roles,
    ...
    snap.namespace("snap.admin").use(["snap.service.groupsService", "snap.service.staffAccountService", "snap.EventAggregator", "snap.admin.schedule.AdminScheduleDSFactory", "snap.common.schedule.ScheduleCommon"])
        .define("AdminSchedulerNavigator", function($groupsService, $staffAccountService, $eventAggregator, $adminScheduleDSFactory, $scheduleCommonObjects) {
            ...
            function Container() {
                ...
                this.listType = "users";
                this.hasGroupsToDisplay = true;
                this.hasUsersToDisplay = true;
                ...
                this.showDetails = function(container) {
                    ...
                    this.trigger("change", {
                        field: "isGroupsListVisible"
                    });
                    this.trigger("change", {
                        field: "isUsersListVisible"
                    });
                    this.trigger("change", {
                        field: "hasParent"
                    });
                ...
                this.toggleTabs = function(e) {
                    ...
                    this.set("activeTabName", $(e.currentTarget).text());
                ...
                this.isNotMatchedVisible = function() {
                    var listHasUsersToDisplay = this.listType === "users" && !this.hasUsersToDisplay;
                    var listHasGroupsToDisplay = this.listType === "groups" && !this.hasGroupsToDisplay;

                    return !this.isNotFoundVisible() && (listHasUsersToDisplay || listHasGroupsToDisplay);
                };

                this.isGroupsListVisible = function() {
                    return this.isGroupsVisible() && !this.isNotMatchedVisible();
                };
                this.isUsersListVisible = function() {
                    return this.isUsersVisible() && !this.isNotMatchedVisible();
                };
                this.openUserTab = function() {
                    this.set("listType", "users");
                    this.set("activeTabName", "Users");
                    this._updateTabState();
                };

                this.hideAllTabs = function() {
                    this.set("listType", "");
                    this.set("activeTabName", "Groups or Users");
                    this._updateTabState();
                };
                ...
                this.getFilteredGroups = function() {
                    var filteredGroups = filterByName(this.subGroups, this.nameFilter);
                    this.hasGroupsToDisplay = filteredGroups.length > 0;
                    return filteredGroups;
                };

                this.getFilteredUsers = function() {
                    var filteredUsers = filterByName(this.members, this.nameFilter);
                    this.hasUsersToDisplay = filteredUsers.length > 0;
                ...
                function saveActionInternal(dfd) {
                    that.save().done(function(result) {
                        ...
                        if (result.total || result.data.length) {
                            ...
                            var message1 = "<span class='localizejs'>" + message + "</span>";
                            $snapNotification.success(message1);
                            // tony.y for #9618
        ...
        function ConsultationDialog(opt, displayOpt) {
            ...
            this._setOptionsForConcreteEventType = function(opt) {
                ...
                if (opt.intakeMetadata) {
                    ...
                    console.log(opt.intakeMetadata.additionalNotes);
                ...
                $.ajax({
                    ...
                    beforeSend: function(request) {
                        if (snap.userSession.apiDeveloperId)
                            request.setRequestHeader("X-Developer-Id", snap.userSession.apiDeveloperId);
                        if (snap.userSession.apiKey)
                            request.setRequestHeader("X-Api-Key", snap.userSession.apiKey);
                        if (snap.userSession.timeZoneSystemId)
                            request.setRequestHeader("Time-Zone", snap.userSession.timeZoneSystemId);
                    }
        ...
        getCodeSetDataSourceReplacingNames: function(codeSetName, hospitalId, replaceNames, replaceByObjects) {
            ...
            return new kendo.data.DataSource({
                transport: {
                    read: function(options) {
                        ds._getCodeSetData(codeSetName, hospitalId).done(function(data) {
                            var lastInOrder = data[data.length - 1].displayOrder;

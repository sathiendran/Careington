...
; (function ($) {
    ...
    var createViewModelForSurgery = function (yearsSrc, sourceItem, listSourceVm) {
        return kendo.observable({
            ...
            GetItem: function () {
                if (this.description === "" || this.month === 0 || this.year === 0 || this.year === "") {
                    return null;
                }
    ...
    var validateSurgeryDate = function (item, dob, validationResult) {
        var ndob = new Date(dob);
        if (item.year < ndob.getFullYear() || (ndob.getFullYear() == item.year && (item.month < (ndob.getMonth() + 1)))) {
            validationResult.valid = false;
            validationResult.message += "Please input valid Surgery date.</br>";
            return true;
        }
        return false;
    };
    ...
    var validateSurgeries = function (isNone, array, dob, validationResult) {
        ...
        if (!isNone) {
            var isEmpty = true;
            for (var i = 0; i < array.length; i++) {
                var item = array[i].GetItem();
                if (item != null) {
                    isEmpty = false;
                    if (validateSurgeryDate(item, dob, validationResult)) {
                        break;
                    }
                }
            }
    ...
    snap.namespace("Snap.Common").use(["snapNotification", "snapHttp", "snapLoader", "Snap.Common.PatientProfileDataSources", "snap.DataService.customerDataService", "Snap.Common.PatientProfileViewModelMode", "invitationService"])
      ...
      .define("PatientProfileEditViewModel", function ($snapNotification, $snapHttp, snapLoader, $ds, $service, $modeEnum, $invitationService) {
          ...
          this.defaultTextFieldValue = "N/A";
          ...
          this.CellPhoneText = function () {
              ...
              var cellPhone = (!code || !phone) ? "N/A" : code + phone;
              return ": " + cellPhone;
          };
          ...
          this.SetUpPhysician = function (name, contact) {
              if (name === noneValue || contact === noneValue) {
                  this.set("primaryPhysician", "");
                  this.set("primaryPhysicianContact", "");
              ...
              this.set("physicianText", this.formatPhysician(name, contact, this.PrimaryPhysicianIsNone));
          ...
          this.SetUpPharmacy = function (name, contact) {
              ...
              this.set("pharmacyText", this.formatPhysician(name, contact, this.PreferedPharmacyIsNone));
          };
          this.SetUpSpecialist = function (name, contact) {
              ...
              this.set("specialistText", this.formatPhysician(name, contact, this.ClinicianSpecialistIsNone));
          };
          this.formatPhysician = function (name, phone, none) {
              if (none) {
                  return "None";
              } else {
                  if (!name && !phone) {
          ...
          this.ethnicityBadgeIsVisible = function () {
              var feth = this.fieldsSetByClinician.ethnicity;
              if (feth == null)
                  return false;
          ...
          this.isEmailVisible = function () {
              if (this.get("isDependentVisible")) {
                  ...
                  if (dob && new Date(dob).toLocalISO() <= snap.dateLimits.getMinDOBforEmail().toLocalISO()) {
                      return true;
                  } else {
                      return false;
                  }
          ...
          this.EditProfile = function (e) {
              ...
              if (!this.isDependentVisible || this.patientId == snap.profileSession.profileId) {
                  ...
                  sessionStorage.setItem("snap_patientId_ref", this.patientId);
                  window.location.href = "/Customer/EditDependent";
              }
          };
          ...
          this.validateInput = function () {
              ...
              if (!this.dob) {
                  ...
                  if (!this.isDependentBool() && new Date(this.dob).toLocalISO() >= snap.dateLimits.getMinDOBforEmail().toLocalISO()) {
                      varvalidationResult.valid = false;
                      var age = new Date().getFullYear() - snap.dateLimits.getMinDOBforEmail().getFullYear();
                      varvalidationResult.message += "Independent user should be at least " + age + " years old </br>";
                  }
              }
              if (!this.gender) {
                  varvalidationResult.valid = false;
              ...
              else if (this.country && this.mobilePhone && this.mobilePhone.length && this.country.length + this.mobilePhone.length > 15) {
                  varvalidationResult.valid = false;
                  varvalidationResult.message += "Cell phone along with code number allow maximum 15 chars <br />";
              }

              if ((emsg = ValidatePhone(getNumbersFromString(this.homePhone), "Home Phone Number", false)).length > 0) {
                  varvalidationResult.valid = false;
          ...
          this.saveChanges = function (e) {
              ...
              if (viewmodel.patientId || viewmodel.isDependentBool()) {
                  ...
                  if (validation.valid) {
                      snapLoader.showLoader();
                      viewmodel.saveChangesInternal({ invitationWasSent: false }).done(function () {
          ...
          this.saveNewPatient = function (preventSendingInvitation) {
              ...
              if (validation.valid) {
                  ...
                  viewmodel.sendPrimaryData(preventSendingInvitation).done(function (resp) {
                      ...
                      viewmodel.saveChangesInternal({ invitationWasSent: !preventSendingInvitation }).done(function () {
                          viewmodel.changesSavedCallback();
                      }).always(function () { that.isSaving = false; snapLoader.hideLoader(); });

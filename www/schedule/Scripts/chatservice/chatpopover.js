...
(function ($) {
    $.fn.popover = function (options) {
        ...
        var floater = $([
              '<div class="popover" id="directChatContainer">'
              , '<div class="chatheader"></div>'
              ,'<div class="chat-loding">Please wait.</div>'
              , '<div class="userStatus">'
                    , '<div class="userStatus"> Status : ' 
                        , '<select>'
        ...
        $('.chatheader', floater).append(header);
        $('.chatcontent', floater).append(content);
        ...
        $(document).bind("click", function (event) {
            if ($.fn.popover.openedPopup != null
                ...
                if ($(event.target).attr("role") == "option") {
                    return;
                }
        ...
        var updateStatusIcon = function (value) {
            $("#spDoctorStatus").removeClass("online").removeClass("busy").removeClass("away").removeClass("offline");
            if (value == "Online") {
                $("#spDoctorStatus").addClass("online")
            } else if (value == "Busy") {
                $("#spDoctorStatus").addClass("busy");
            } else if (value == "Away") {
                $("#spDoctorStatus").addClass("away");
            } else if (value == "Offline") {
                $("#spDoctorStatus").addClass("offline");
            }
        };
        ...
        var showPopover = function (button) {
            ...
            var $chatHub = snap.hub.ChatHub();
           
            triangleSize = 0;
           
            topOff = offset.top + buttonHeight;;
            ...
            window.setTimeout(function () {
                ...
                $(window).resize();
                var selElement = floater.find("select");
                if (selElement.data("kendoDropDownList")) {
                   
                    return;
                }
                floater.find("select").kendoDropDownList({
                    change: function () {
                        var value = floater.find("select").val();
                       
                        if ($chatHub) {
                            $chatHub.changeStatus(value);
                        } else {
                            if (snap.eventAggregator) {
                                snap.eventAggregator.published("physicianstatuschange", value);
                            }
                        }
                        updateStatusIcon(value);
                    }
                });
                var status = localStorage.getItem("physicianstatus");
                var cmb = floater.find("select").data("kendoDropDownList");
                if (cmb) {
                    cmb.select(function (item) {
